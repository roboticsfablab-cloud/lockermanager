...
module.exports = function (db) {
    ...
    router.patch('/items/:id/custody', async (req, res) => {
        try {
            ...
            if (sets.length) {
                ...
                await db.execute({ sql: `UPDATE covenant_history SET ${sets.join(', ')} WHERE item_id=? AND entity_type='warehouse_item' AND status='active'`, args });
            }
            res.json({ success: true });
        } catch (e) { res.status(500).json({ error: e.message }); }
    });

    // Return an active custody entry to a chosen destination — a real move,
    // not just a status flip. Returning to the item's own zone/area is a
    // no-op move (stock never left); any other zone/area or a locker
    // relocates the (possibly partial) quantity there first. Distinct from
    // /custody/return, which always returns to the current spot.
    router.post('/items/:id/custody/return-to', async (req, res) => {
        try {
            const { destination_type, locker_id, zone_id, area_id, qty, return_condition, return_notes } = req.body;
            const item = await db.execute({ sql: 'SELECT * FROM warehouse_items WHERE id = ?', args: [req.params.id] });
            if (item.rows.length === 0) return res.status(404).json({ error: 'Item not found' });
            const wi = item.rows[0];
            const active = await db.execute({ sql: "SELECT * FROM covenant_history WHERE item_id=? AND entity_type='warehouse_item' AND status='active' ORDER BY id DESC LIMIT 1", args: [req.params.id] });
            if (active.rows.length === 0) return res.status(400).json({ error: 'No active custody to return' });
            const cov = active.rows[0];

            const available = Number(wi.qty) || 0;
            if (available < 1) return res.status(400).json({ error: 'No stock available to return' });
            const requested = parseInt(qty);
            const fallback = Number(cov.qty) || available;
            const moveQty = Math.min(available, Math.max(1, isNaN(requested) ? fallback : requested));

            const sameZone = destination_type === 'warehouse' && Number(zone_id) === Number(wi.zone_id)
                && (area_id ? Number(area_id) === Number(wi.area_id) : !wi.area_id);
            if (!sameZone) {
                if (destination_type === 'locker') {
                    ...
                    const existing = await db.execute({ sql: 'SELECT id FROM items WHERE locker_id = ? AND name = ?', args: [locker_id, wi.name] });
                    if (existing.rows.length) await db.execute({ sql: 'UPDATE items SET qty = qty + ? WHERE id = ?', args: [moveQty, existing.rows[0].id] });
                    else await db.execute({ sql: 'INSERT INTO items (locker_id, name, qty, description, image) VALUES (?, ?, ?, ?, ?)', args: [locker_id, wi.name, moveQty, wi.description || '', wi.image || ''] });
                    ...
                    const existing = area_id
                        ? await db.execute({ sql: 'SELECT id FROM warehouse_items WHERE zone_id = ? AND area_id = ? AND name = ?', args: [zone_id, area_id, wi.name] })
                        : await db.execute({ sql: 'SELECT id FROM warehouse_items WHERE zone_id = ? AND area_id IS NULL AND name = ?', args: [zone_id, wi.name] });
                    if (existing.rows.length) await db.execute({ sql: 'UPDATE warehouse_items SET qty = qty + ? WHERE id = ?', args: [moveQty, existing.rows[0].id] });
                    else await db.execute({ sql: 'INSERT INTO warehouse_items (zone_id, area_id, name, qty, description, image, condition) VALUES (?, ?, ?, ?, ?, ?, ?)', args: [zone_id, area_id || null, wi.name, moveQty, wi.description || '', wi.image || '', wi.condition || 'new'] });
                } else {
                    return res.status(400).json({ error: 'destination_type must be locker or warehouse' });
                }

                const remaining = available - moveQty;
                if (remaining <= 0) await db.execute({ sql: 'DELETE FROM warehouse_items WHERE id = ?', args: [wi.id] });
                else await db.execute({ sql: 'UPDATE warehouse_items SET qty = ? WHERE id = ?', args: [remaining, wi.id] });
            }

            const today = new Date().toISOString().split('T')[0];
            await db.execute({
                sql: "UPDATE covenant_history SET status='returned', return_condition=?, return_notes=?, end_date=? WHERE id=?",
                args: [return_condition || '', return_notes || '', today, cov.id]
            });
